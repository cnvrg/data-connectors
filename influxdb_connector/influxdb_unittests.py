...
class TestInfluxdbQuery(TestInfluxdb):
    def test_return_type(self):
        ...
        self.assertIsInstance(influxdb_query(self.args[0],self.args[1],self.args[2],self.args[3],self.args[4]),dict)

class TestCustomBucketFormatting(TestInfluxdb):
    def __init__(self):
        self.csv_builder = influxdb_query(self.args[0],self.args[1],self.args[2],self.args[3],self.args[4])

    def test_return_type(self):
        """Checks if the function returns dictionary"""
        self.assertIsInstance(custom_bucket_formatting(self.csv_builder,self.args[3]),dict)